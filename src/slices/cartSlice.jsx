...
const cartSlice = createSlice({
    ...
    reducers: {
        ...
        updateCart: (state, action) => {
            const plat = (state.data).find(t => t._id === action.payload._id);
        ...
        deleteCart: (state, action) => {
            // state = (state.data).filter((t) => t._id !== action.payload);
            ...
            // });
            // return state;
            const updatedData = state.data.filter((item, id) => item.id !== action.payload);
            toast.success("Produit supprimé avec succès", {
                autoClose: 700,
                theme: 'light',
                pauseOnHover: false,
